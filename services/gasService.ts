...
class GasService {
  ...
  // Fecha de nacimiento y celular de los pasajeros (máx. 5 cédulas): solo indica
  // si cada persona está registrada y si ya los tiene. Nunca devuelve los datos.
  ...
  // Solo administradores: fecha de nacimiento y celular de cada pasajero de una
  // solicitud. Respaldo: desde #A74 el detalle los recibe dentro de getRequestById.
  async getPassengerBirthdates(requestId: string): Promise<PassengerAdminInfo[]> {
    const response = await this.runGas('getPassengerBirthdates', { requestId });
    if (!response.success) throw new Error(response.error);
    return response.data as PassengerAdminInfo[];
  }
